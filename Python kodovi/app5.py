...
class GrapplerUltraApp(ctk.CTk):
    def __init__(self):
        ...
        # Na različitim računarima ekrani imaju različite rezolucije i scaling.
        # Zoomed mod koristi maksimalan raspoloživ prostor bez rušenja layouta.
        try:
            self.state("zoomed")
        except tk.TclError:
            pass
    ...
    def build_sidebar(self):
        self.sidebar = ctk.CTkScrollableFrame(self, width=335, corner_radius=0, fg_color="#0b1220")
        ...
        for name, value in hardware_rows:
            row = ctk.CTkFrame(self.hardware_info, fg_color="transparent")
    ...
    def draw_hardware_map(self):
        ...
        # Namjerno je sve pomjereno prema sredini da tekst ne izlazi iz sidebar-a
        # na računarima sa većim Windows scalingom.
        board_x1, board_y1 = 78, 22
        board_x2, board_y2 = 208, 175
        c.create_rectangle(board_x1, board_y1, board_x2, board_y2, outline="#334155", width=2, fill="#0b1120")
        c.create_text(143, 42, text="ARDUINO UNO", fill="#e5e7eb", font=("Consolas", 11, "bold"))

        pins = [
            ("D10 RX", 64, 72, "pin_d10", "left"),
            ("D11 TX", 64, 102, "pin_d11", "left"),
            ("D6 S1", 222, 72, "pin_d6", "right"),
            ("D5 S2", 222, 102, "pin_d5", "right"),
            ("GND", 143, 154, "pin_gnd", "center"),
        ]

        for text, x, y, tag, side in pins:
            color = "#475569" if tag != "pin_gnd" else "#22c55e"
            c.create_oval(x - 7, y - 7, x + 7, y + 7, fill=color, outline="", tags=tag)

            if side == "left":
                c.create_text(12, y, text=text, fill="#cbd5e1", anchor="w", font=("Consolas", 10))
                c.create_line(72, y, board_x1, y, fill="#334155", width=1)
            elif side == "right":
                c.create_text(236, y, text=text, fill="#cbd5e1", anchor="w", font=("Consolas", 10))
                c.create_line(board_x2, y, 215, y, fill="#334155", width=1)
            else:
                c.create_text(x, y + 16, text=text, fill="#cbd5e1", anchor="center", font=("Consolas", 10))

        c.create_text(143, 200, text="Pinovi svijetle pri aktivnosti", fill="#64748b", font=("Consolas", 9))
    ...
    def draw_gauges(self):
        c = self.gauge_canvas
        c.delete("all")
        self.draw_gauge(c, 78, 76, 43, self.grip_intensity, "GRIP", "#22c55e")
        self.draw_gauge(c, 207, 76, 43, 82 if self.serial.connected() else 0, "LINK", "#60a5fa")
