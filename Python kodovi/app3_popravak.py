...
class GrapplerUltraApp(ctk.CTk):
    ...
    def build_controls(self):
        ...
        self.buttons = []
        self.command_button("UHVATI", "g", "Start sekvence", "#2563eb", "#1d4ed8", 0)
        self.command_button("E-STOP / PUSTI", "f", "Sigurno zaustavljanje", "#dc2626", "#b91c1c", 1)
        self.command_button("HOME", "h", "Početni položaj", "#334155", "#475569", 2)
        self.command_button("OTVORI", "o", "Otpuštanje", "#0f766e", "#115e59", 3)
        self.command_button("ZATEGNI", "c", "Finalni grip", "#7c3aed", "#6d28d9", 4)
        self.command_button("TEST S1", "1", "Servo 1", "#1f2937", "#374151", 5)
        self.command_button("TEST S2", "2", "Servo 2", "#1f2937", "#374151", 6)
        self.command_button("PING", "?", "Latency test", "#1f2937", "#374151", 7)
    ...
    def build_mid_panels(self):
        ...
        self.calib_card = ctk.CTkFrame(self.mid, corner_radius=22, fg_color="#111827")
        ...
        self.speed_frame = ctk.CTkFrame(self.calib_card, fg_color="#0f172a", corner_radius=16)
    ...
    def build_log_panel(self):
        ...
        self.record_btn = ctk.CTkButton(self.log_header, text="REC MACRO", width=110, fg_color="#7c3aed", hover_color="#6d28d9", command=self.toggle_recording)
        self.record_btn.grid(row=0, column=2, padx=6)

        self.play_btn = ctk.CTkButton(self.log_header, text="PLAY MACRO", width=120, fg_color="#0f766e", hover_color="#115e59", command=self.play_macro)
        self.play_btn.grid(row=0, column=3, padx=6)

        self.export_btn = ctk.CTkButton(self.log_header, text="EXPORT", width=90, fg_color="#374151", hover_color="#4b5563", command=self.export_logs)
        self.export_btn.grid(row=0, column=4, padx=6)

        self.clear_btn = ctk.CTkButton(self.log_header, text="CLEAR", width=80, fg_color="#374151", hover_color="#4b5563", command=self.clear_log)
    ...
    def slider_row(self, label, from_, to, initial, callback):
        frame = ctk.CTkFrame(self.calib_card, fg_color="#0f172a", corner_radius=16)
    ...
    def send_command(self, cmd, description, record=True):
        ...
        try:
            ...
            if cmd.startswith("A") or cmd.startswith("B"):
                pass
            else:
