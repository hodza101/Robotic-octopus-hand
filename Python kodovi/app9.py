...
class GrapplerApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        self.main.grid_rowconfigure(4, weight=1, minsize=210)
    ...
    def build_main(self):
        ...
        self.twin_canvas = ctk.CTkCanvas(twin, height=260, bg=COLORS["card2"], highlightthickness=0)
        self.twin_canvas.pack(fill="both", expand=True, padx=14, pady=(0, 14))

        self.build_calibration_panel()

        log_frame = ctk.CTkFrame(self.main, fg_color=COLORS["card"], corner_radius=18, height=220)
        log_frame.grid(row=4, column=0, columnspan=4, sticky="nsew")
        log_frame.grid_propagate(False)
        log_frame.grid_columnconfigure(0, weight=1)
        log_frame.grid_rowconfigure(1, weight=1, minsize=145)

        top = ctk.CTkFrame(log_frame, fg_color="transparent", height=44)
        top.grid(row=0, column=0, sticky="ew", padx=18, pady=(12, 6))
        ...
        ctk.CTkButton(button_bar, text="CLEAR", width=70, height=30, command=self.clear_log, fg_color=COLORS["disabled"]).pack(side="left", padx=4)
        ctk.CTkButton(button_bar, text="EXPORT", width=80, height=30, command=self.export_log, fg_color=COLORS["disabled"]).pack(side="left", padx=4)
        ctk.CTkButton(button_bar, text="PLAY MACRO", width=100, height=30, command=self.play_macro).pack(side="left", padx=4)
        ctk.CTkButton(button_bar, text="REC MACRO", width=95, height=30, command=self.toggle_record).pack(side="left", padx=4)
    ...
    def build_calibration_panel(self):
        ...
        ctk.CTkLabel(
            calib,
            text="Sigurna logika: slider prvo mijenja preview. Servo se pomjera kada klikneš PRIMIJENI ili kada je LIVE uključen.",
            text_color=COLORS["muted"],
            font=("Arial", 11),
            wraplength=540,
            justify="left",
        ...
        quick.pack(fill="x", padx=18, pady=(10, 12))
        ...
        self.live_hint.pack(anchor="w", padx=18, pady=(0, 12))
    ...
    def calibration_slider(self, parent, name, pin, value, callback):
        ...
        slider.set(value)
        slider.grid(row=1, column=0, sticky="ew", padx=14, pady=(8, 12))

        marks = ctk.CTkFrame(box, fg_color="transparent")
        marks.grid(row=2, column=0, sticky="ew", padx=14, pady=(0, 10))
    ...
    def command_button(self, parent, text, cmd, color, row, col, subtitle):
        ...
        box.grid(row=row, column=col, sticky="ew", padx=18, pady=10)
        button = ctk.CTkButton(box, text=text, command=lambda: self.send_command(cmd), height=42, fg_color=color, font=("Arial", 13, "bold"))
        button.pack(fill="x")
        ctk.CTkLabel(box, text=subtitle, text_color=COLORS["muted"], font=("Arial", 10)).pack(pady=(5, 0))
