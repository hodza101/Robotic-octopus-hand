...
class SpiRobDesktopApp(ctk.CTk):
    ...
    def build_sidebar(self):
        ...
        self.hardware_canvas = ctk.CTkCanvas(hardware, width=210, height=160, bg=COLORS["card"], highlightthickness=0)
    ...
    def build_work_area(self):
        ...
        twin.grid(row=0, column=0, sticky="nsew", padx=(0, 6))

        ctk.CTkLabel(twin, text="Digital Twin / Wireframe", font=("Arial", 18, "bold")).pack(anchor="w", padx=18, pady=(14, 8))
        self.twin_canvas = ctk.CTkCanvas(twin, height=300, bg=COLORS["card_dark"], highlightthickness=0)
    ...
    def draw_hardware_map(self, active):
        ...
        c.create_rectangle(65, 22, 145, 124, outline="#4b5563", width=2)
        c.create_text(105, 62, text="STM32\nF103C8T6", fill="white", font=("Consolas", 10, "bold"))
        ...
        for x, y, label, side in pins:
            fill = "#60a5fa" if active else "#64748b"
            c.create_oval(x - 6, y - 6, x + 6, y + 6, fill=fill, outline="")
            if side == "left":
                c.create_text(x - 10, y, text=label, fill="white", font=("Consolas", 8), anchor="e")
            else:
                c.create_text(x + 10, y, text=label, fill="white", font=("Consolas", 8), anchor="w")

        c.create_oval(99, 108, 111, 120, fill=COLORS["green"], outline="")
        c.create_text(105, 142, text="GND", fill="white", font=("Consolas", 8))
    ...
    def draw_digital_twin(self):
        ...
        width = max(c.winfo_width(), 500)
        height = max(c.winfo_height(), 260)

        cx = width / 2
        cy = height / 2 + 8
        ratio = self.grip / 100.0
        ...
        for i in range(6):
            ...
            curl = math.radians(20 + 55 * ratio)

            x0 = cx + math.cos(a) * 62
            y0 = cy + math.sin(a) * 62
            x1 = x0 + math.cos(a + curl) * 76
            y1 = y0 + math.sin(a + curl) * 76
